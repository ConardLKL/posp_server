...
public final class PacketDefineInitializer {
	...
	private PacketDefineInitializer() {
	}

	@SuppressWarnings("unused")
	private synchronized void init() throws Exception {
		TCfgPktDef def = new TCfgPktDef();
		def.setChannelCode(SysConstant.CAPITAL_POOL_7001);
		Map<String, List<TCfgPktDef>> pktDefMap = tCfgPktDefService.initPktDef(def);
		
		for (Entry<String, List<TCfgPktDef>> pktItem : pktDefMap.entrySet()) {
